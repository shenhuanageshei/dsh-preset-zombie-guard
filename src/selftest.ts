...
function zstdFrame(lines: string[]): Buffer {
...
function headerLine(spec: HeaderSpec): string {
  ...
  if (spec.agentPreset !== undefined) record['agentPreset'] = spec.agentPreset
  return JSON.stringify(record)
}

function event(type: string, seq: number, data?: Record<string, unknown>): string {
  const record: Record<string, unknown> = { type, seq, time: 1787000000000 + seq }
...
function writeZstdSession(
  ...
  for (const batch of batches) parts.push(zstdFrame(batch))
...
function recordingLogger(): { logger: Logger; info: string[]; warn: string[]; error: string[] } {
  const info: string[] = []
  const warn: string[] = []
  const error: string[] = []
  return {
...
function mockServices(opts: {
  roster?: string[]
  archived?: string[]
  live?: (id: string) => boolean | undefined
}): { services: ScanServices; archiveCalls: string[] } {
...
function scanOptionsFor(root: string, logger: Logger, overrides: Partial<ScanOptions> = {}): ScanOptions {
  return {
    root,
...
async function main(): Promise<number> {
  ...
  check(
    '(unit) segment 解码',
    decodeSegmentName('session-abc~0020def') === 'session-abc def' && decodeSegmentName('~002E') === '.',
  )
  ...
  try {
    // ─── (a) 僵尸空白：zstd 与 jsonl 双路径，A 类自动归档 ─────────────────────
    {
      ...
      check(
        '(a) 归档动作执行',
        archiveCalls.length === 2 && report.archived.length === 2 && report.archived.includes(zstdId) && report.archived.includes(jsonlId),
        `calls=${JSON.stringify(archiveCalls)} archived=${JSON.stringify(report.archived)}`,
      )
    ...
    // ─── (a2) v1.3 判别用例：id-α 真实形状（创建生命周期事件 ≠ 对话活动）─────────
    {
      ...
      const path = writeZstdSession(
        root,
        '--ws-a2--',
        a2Id,
        { id: a2Id, cwd: 'D:\\ws-a2', agentPreset: 'ghost-preset' },
      ...
      check(
        '(a2) 三个创建生命周期事件全在白名单 → blank=true',
        !parsed.decodeFail && parsed.blank === true && parsed.effectivePreset === 'ghost-preset' && parsed.eventCount === 3,
      ...
      const report = await new ZombieGuardEngine().scanOnce(services, scanOptionsFor(root, rec.logger))
      check(
        '(a2) 僵尸空白 → A 类候选并自动归档（v1.3 白名单扩表）',
        report.classes.A === 1 && archiveCalls.length === 1 && archiveCalls[0] === a2Id && report.warnings.length === 0,
        `A=${report.classes.A} calls=${JSON.stringify(archiveCalls)} warnings=${report.warnings.length}`,
      )
    }
    ...
    // ─── (a3) 扩表不放过真对话：同 (a2) + 一条 user/message → 非空白 ────────────
    {
      ...
      writeZstdSession(
        ...
        '--ws-a3--',
        a3Id,
        { id: a3Id, cwd: 'D:\\ws-a3', agentPreset: 'ghost-preset' },
        [
          [
            event('permission/preset', 1, { preset: 'workspace-write' }),
            event('sandbox/mode', 2, { mode: 'workspace-write' }),
      ...
      const report = await new ZombieGuardEngine().scanOnce(services, scanOptionsFor(root, rec.logger))
      check(
        '(a3) 含 user/message → 僵尸非空白（B 类告警，不归档）',
        report.classes.A === 0 &&
          report.classes.B === 1 &&
        ...
        `classes=${JSON.stringify(report.classes)} calls=${JSON.stringify(archiveCalls)}`,
      )
    }
    ...
    // ─── (b) 僵尸非空白 + check_remove β 类判别（header-only 会漏）────────────
    {
      ...
      writeZstdSession(
        ...
        '--ws-b--',
        bId,
        { id: bId, cwd: 'D:\\ws-b', agentPreset: 'standard' },
        [[event('step/start', 1)], [event('agent-preset/selected', 2, { agentPreset: 'ghost-preset' }), event('step/end', 3)]],
      )
      const rec = recordingLogger()
      const { services, archiveCalls } = mockServices({ roster: ROSTER })
      ...
      check(
        '(b) check_remove 列出 β 类（header-only 检查会漏）',
        cr.matches.length === 1 &&
          cr.matches[0].id === bId &&
          cr.matches[0].dependsOn.length === 1 &&
          cr.matches[0].dependsOn[0] === 'effective(strong)',
        `matches=${JSON.stringify(cr.matches)}`,
      )
      ...
      check(
        '(b) check_remove 弱依赖标注',
        crStandard.matches.length === 1 && crStandard.matches[0].dependsOn.includes('header(weak)'),
        `matches=${JSON.stringify(crStandard.matches)}`,
      )
    }
    ...
    // ─── (c) 截断 zstd 帧：decodeFail、扫描继续、横幅 ──────────────────────────
    {
      ...
      const path = writeZstdSession(
        ...
        '--ws-c--',
        cId,
        { id: cId, cwd: 'D:\\ws-c', agentPreset: 'ghost-preset' },
        [[event('session/title', 1, { title: 't' })]],
      )
      ...
      const report = await new ZombieGuardEngine().scanOnce(services, scanOptionsFor(root, rec.logger))
      check(
        '(c) 截断帧计 decodeFail、扫描继续',
        report.scanned === 2 && report.decodeFailCount === 1 && report.classes.C === 1 && report.classes.D === 1,
        `scanned=${report.scanned} decodeFails=${report.decodeFailCount} classes=${JSON.stringify(report.classes)}`,
      )
      check(
        '(c) 报告含不可抑制不完整横幅',
        report.banner !== undefined && report.banner.includes('未能完整解码'),
        `banner=${String(report.banner)}`,
      )
    ...
    // ─── (c2) header 健康的 decodeFail 也计入未完整横幅（v1.3 口径）──────────────
    {
      ...
      const path = writeZstdSession(
        ...
        { id: c2Id, cwd: 'D:\\ws-c2', agentPreset: 'standard' },
        [[event('session/title', 1, { title: 't' })]],
      )
      const raw = readFileSync(path)
      const frameBoundary = scanZstdFrames(raw).frames[0].end
      ...
      const report = await new ZombieGuardEngine().scanOnce(services, scanOptionsFor(root, rec.logger))
      check(
        '(c2) header 健康的 decodeFail 不入 C 类但计入横幅（全部 decodeFail 口径）',
        report.decodeFailCount === 1 &&
          ...
          report.banner.includes('1 个会话未能完整解码'),
        `decodeFails=${report.decodeFailCount} classes=${JSON.stringify(report.classes)} banner=${String(report.banner)}`,
      )
      check(
        '(c2) 不产生告警/归档/异常',
        archiveCalls.length === 0 && rec.warn.length === 0 && rec.error.length === 0,
      )
    }
    ...
    // ─── (d) report-only：autoArchiveBlank=false 与 override 模式 ─────────────
    {
      ...
      const rec = recordingLogger()
      const { services, archiveCalls } = mockServices({ roster: ROSTER })
      const report = await new ZombieGuardEngine().scanOnce(
        services,
        scanOptionsFor(root, rec.logger, { autoArchiveBlank: false }),
      )
      check(
        '(d1) autoArchiveBlank=false：分类正确且归档调用为 0',
        report.classes.A === 1 && archiveCalls.length === 0 && report.archiveEnabled === false && report.warnings.length === 1,
        `A=${report.classes.A} calls=${archiveCalls.length} warnings=${report.warnings.length}`,
      )
    }
    {
      ...
      const report = await new ZombieGuardEngine().scanOnce(
        services,
        scanOptionsFor(root, rec.logger, { overrideMode: true }),
      )
      check(
        '(d2) override 模式：归档整体禁用',
        report.classes.A === 1 && archiveCalls.length === 0 && report.archiveEnabled === false && report.overrideMode === true,
        `A=${report.classes.A} calls=${archiveCalls.length}`,
      )
    }
    ...
    // ─── (e) .bak-* 修复备份被完全忽略 ────────────────────────────────────────
    {
      ...
      check(
        '(e) .bak 备份被完全忽略',
        report.scanned === 1 && report.classes.D === 1 && report.warnings.length === 0 && report.decodeFailCount === 0,
        `scanned=${report.scanned} classes=${JSON.stringify(report.classes)}`,
      )
    ...
    // ─── (f) header 无 agentPreset 且无 selected 事件 → 无预设依赖跳过 ─────────
    {
      ...
      check(
        '(f) 无预设依赖会话跳过且永不判僵尸',
        report.noPresetDependency === 2 &&
          report.classes.A === 0 &&
          report.classes.B === 0 &&
          report.classes.C === 0 &&
          report.warnings.length === 0,
        `noPreset=${report.noPresetDependency} classes=${JSON.stringify(report.classes)}`,
      )
    }
    ...
    // ─── (g) 缓存命中：第二次扫描零重解码 ─────────────────────────────────────
    {
      ...
      check(
        '(g) 首扫全量解码',
        first.decodes === 2 && first.cacheHits === 0,
        `decodes=${first.decodes} hits=${first.cacheHits}`,
      )
      check(
        '(g) 二扫零重解码（缓存命中）',
        second.decodes === 0 && second.cacheHits === 2,
        `decodes=${second.decodes} hits=${second.cacheHits}`,
      )
      check(
        '(g) 二扫分类与首扫一致',
        second.classes.A === first.classes.A && second.classes.D === first.classes.D,
        `first=${JSON.stringify(first.classes)} second=${JSON.stringify(second.classes)}`,
      )
    }
    ...
    // ─── (h) 多事件单帧完整解析（防「整帧当单条 JSON」回归）─────────────────────
    {
      ...
      const path = writeZstdSession(
        ...
        '--ws-h--',
        hId,
        { id: hId, cwd: 'D:\\ws-h', agentPreset: 'standard' },
      ...
      check(
        '(h) 单帧 3 事件全部解析',
        parsed.eventCount === 3 && parsed.hasSelected && parsed.effectivePreset === 'ghost-preset' && parsed.blank === false && !parsed.decodeFail,
        `parsed=${JSON.stringify(parsed)}`,
      )
      const rec = recordingLogger()
      const { services } = mockServices({ roster: ROSTER })
    ...
    // ─── (i) agentPresets 缺失 → 整轮跳过并 warn ──────────────────────────────
    {
      ...
      check(
        '(i) 无 roster 整轮跳过',
        report.skippedReason !== undefined && report.scanned === 0 && rec.warn.length === 1,
        `skipped=${String(report.skippedReason)} warn=${rec.warn.length}`,
      )
    }
    ...
    // ─── (j) liveness 不可判定 → 命中 A 不归档，标注 live? unknown ─────────────
    {
      ...
      const report = await new ZombieGuardEngine().scanOnce(services, scanOptionsFor(root, rec.logger))
      check(
        '(j) liveness 不可判定不归档、按 B 告警标注',
        report.classes.A === 1 &&
          archiveCalls.length === 0 &&
          report.warnings.length === 1 &&
          report.warnings[0].note !== undefined &&
          report.warnings[0].note.includes('live? unknown'),
        `A=${report.classes.A} calls=${archiveCalls.length} warnings=${JSON.stringify(report.warnings)}`,
      )
    }
    ...
    // ─── (k) zstd 能力缺失 → zstd 日志按解码失败、归档禁用、横幅仍在 ───────────
    {
      ...
      const report = await new ZombieGuardEngine().scanOnce(
        services,
        scanOptionsFor(root, rec.logger, { zstdCapable: false }),
      )
      check(
        '(k) zstd 缺失：zstd 会话 decodeFail、jsonl 正常、归档禁用',
        report.decodeFailCount === 1 &&
          report.classes.C === 1 &&
          report.classes.D === 1 &&
          ...
          archiveCalls.length === 0 &&
          report.banner !== undefined,
        `decodeFails=${report.decodeFailCount} classes=${JSON.stringify(report.classes)}`,
      )
    }
    ...
    // ─── (l) 已归档会话幂等跳过 ────────────────────────────────────────────────
    {
      ...
      check(
        '(l) 已归档僵尸空白幂等跳过',
        report.classes.A === 1 && report.alreadyArchived === 1 && archiveCalls.length === 0 && report.warnings.length === 0,
        `A=${report.classes.A} already=${report.alreadyArchived} calls=${archiveCalls.length}`,
      )
    }
    ...
    // ─── (m) live 会话在碰文件前跳过 ──────────────────────────────────────────
    {
      ...
      check(
        '(m) live 会话跳过且不解码',
        report.scanned === 1 && report.liveSkipped === 1 && report.decodes === 0 && report.classes.A === 0,
        `scanned=${report.scanned} liveSkipped=${report.liveSkipped} decodes=${report.decodes}`,
      )
    }
    ...
    // ─── (n) B 类告警去重 + acknowledgedSessions 静默 ──────────────────────────
    {
      ...
      writeZstdSession(
        ...
        '--ws-n--',
        id,
        { id, cwd: 'D:\\ws-n', agentPreset: 'standard' },
        [[event('step/start', 1)], [event('agent-preset/selected', 2, { agentPreset: 'ghost-preset' })]],
      )
      const rec = recordingLogger()
      const { services } = mockServices({ roster: ROSTER })
      const engine = new ZombieGuardEngine()
      ...
      check(
        '(n) 每 (id,状态) 只 warn 一次',
        first.newWarnings === 1 && second.newWarnings === 0 && rec.warn.length === 1,
        `first=${first.newWarnings} second=${second.newWarnings} warns=${rec.warn.length}`,
      )
      check('(n) 仍命中仍列出（只是不重复 warn）', second.warnings.length === 1 && second.classes.B === 1)
      const third = await engine.scanOnce(
        services,
        scanOptionsFor(root, rec.logger, { acknowledged: new Set([id]) }),
      )
      check(
        '(n) acknowledged 永久静默',
        third.silencedByAck === 1 && third.warnings.length === 0 && rec.warn.length === 1,
        `silenced=${third.silencedByAck} warns=${rec.warn.length}`,
      )
    }
  } finally {
