...
/** 宿主服务的最小结构面（通过 ctx.get 动态获取，全部 optional）。 */
...
export function apply(ctx: AppContext, config: Config): void {
  ...
  async function runScan(): Promise<ScanReport> {
    ...
    const agents = ctx.get('agents') as LookupService | undefined
    const sessions = ctx.get('sessions') as LookupService | undefined

    let roster: ReadonlySet<string> | undefined
    ...
    let archivedIds: ReadonlySet<string> | undefined
      ...
      isLive(id) {
        ...
        try {
          if (agents !== undefined) {
            ...
            if (agents.get(id) != null) return true
            known = true
          }
        } catch {
          /* 单服务查询失败按该服务不可用 */
        ...
        try {
          if (sessions !== undefined) {
            if (sessions.get(id) != null) return true
            known = true
          }
        } catch {
          /* 同上 */
  ...
  function formatScanReport(r: ScanReport): string {
    const lines: string[] = []
    ...
    lines.push(
      `会话: scanned=${r.scanned} liveSkipped=${r.liveSkipped} cacheHits=${r.cacheHits} decodes=${r.decodes} zstd=${r.zstdCapable ? 'ok' : '缺失'}`,
    )
    lines.push(
      `命中: A僵尸空白=${r.classes.A} B僵尸非空白=${r.classes.B} C未定=${r.classes.C} D健康=${r.classes.D} 无预设依赖=${r.noPresetDependency} 已归档跳过=${r.alreadyArchived} 解码失败=${r.decodeFailCount}`,
    )
    if (r.archived.length > 0) lines.push(`已归档: ${r.archived.join(', ')}`)
    if (r.archiveFailed.length > 0) lines.push(`归档失败(已记error): ${r.archiveFailed.join(', ')}`)
    ...
    for (const w of r.warnings) {
      ...
      if (w.decodeFail) parts.push(`解码失败(${w.reason ?? '?'})`)
      if (w.note !== undefined) parts.push(w.note)
      if (!w.firstWarned) parts.push('（已告警过，不重复）')
      lines.push(parts.join(' '))
    }
    if (r.banner !== undefined) lines.push(`⚠️ ${r.banner}`)
    return lines.join('\n')
  }

  function formatCheckRemoveReport(r: CheckRemoveReport): string {
    const lines: string[] = []
    ...
    if (r.matches.length === 0) {
      ...
      for (const m of r.matches) {
        ...
        if (m.decodeFail) parts.push(`（该会话解码失败 ${m.reason ?? '?'}，effective 不可知）`)
        lines.push(parts.join(' '))
      }
    }
    ...
    () =>
      ctx.tools.register(
        defineTool({
          ...
          description:
            '删除预设前的依赖安全检查：全量解码所有会话日志，列出依赖指定 presetId 的会话（effective=强依赖，header=弱依赖）。使用纪律：删除任何用户预设前必须先跑本工具，且必须确认输出无「未完整分析」横幅才可视为安全。',
          parameters: {
          ...
          async execute(args: { presetId: string }) {
            const presetId = args.presetId.trim()
            if (presetId === '') return 'ERROR: presetId 不能为空'
  ...
  logger.info(
    `守护启动：每 ${config.intervalMs}ms 扫描 ${scanRoot}${override !== '' ? '（override，归档禁用）' : ''}` +
