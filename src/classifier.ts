 ...
 */
export function isBenignType(type: string): boolean {
  return (
...
function newYieldPolicy(): YieldPolicy {
  return { lastYield: Date.now() }
}

async function maybeYield(policy: YieldPolicy): Promise<void> {
...
function emptyParsed(reason: string): ParsedLog {
...
//#region zstd 帧扫描（语义对齐 dsh-session-persistence-jsonl 的 scanZstdFrames，改为不抛出）
...
export function scanZstdFrames(buffer: Buffer): FrameScanResult {
  ...
  while (offset < buffer.length) {
    ...
    if (buffer.readUInt32LE(offset) !== ZSTD_MAGIC) return { frames, error: `invalid frame magic at byte ${offset}` }
    offset += 4
    if (offset === buffer.length) return { frames, tornStart: start }
    const descriptor = buffer.readUInt8(offset)
    offset += 1
    if ((descriptor & 24) !== 0) return { frames, error: `reserved frame-header bit at byte ${offset - 1}` }
    ...
    for (;;) {
      ...
      if (blockType === 3) return { frames, error: `reserved block type at byte ${offset - 3}` }
...
async function consumeLines(text: string, state: ParseState, policy?: YieldPolicy): Promise<void> {
  const lines = text.split('\n')
  for (const line of lines) {
    if (line.length === 0) continue
    consumeLine(line, state)
    if (policy !== undefined && Date.now() - policy.lastYield >= YIELD_INTERVAL_MS) await maybeYield(policy)
  }
}

function consumeLine(line: string, state: ParseState): void {
  let parsed: unknown
  try {
    parsed = JSON.parse(line)
  } catch {
    state.decodeFail = true
    if (state.decodeFailReason === undefined) state.decodeFailReason = 'bad-json-line'
    return
  }
  if (!isRecord(parsed)) {
    state.decodeFail = true
    if (state.decodeFailReason === undefined) state.decodeFailReason = 'non-object-line'
    return
  }
  if (!state.headerSeen) {
    if (parsed['type'] !== 'session') {
      state.decodeFail = true
      if (state.decodeFailReason === undefined) state.decodeFailReason = 'bad-header'
      return
    }
    ...
    if (typeof id !== 'string' || id.length === 0) {
      state.decodeFail = true
      if (state.decodeFailReason === undefined) state.decodeFailReason = 'header-no-id'
    } else {
      state.headerId = id
    }
    if (typeof cwd === 'string') state.headerCwd = cwd
    if (typeof preset === 'string') state.headerPreset = preset
    return
  }
  ...
  if (type === 'agent-preset/selected') {
    ...
    if (typeof preset === 'string') {
      ...
      state.decodeFail = true
      if (state.decodeFailReason === undefined) state.decodeFailReason = 'malformed-selected'
    }
  }
  if (type === 'step/start') state.sawStepStart = true
  if (!isBenignType(type)) state.allBenign = false
}

/** 汇总解析状态为 ParsedLog；decodeFail 时 effective/blank 置为不可信。 */
function finalizeParse(state: ParseState): ParsedLog {
  if (state.decodeFail) {
    return {
...
async function decodeZstdLog(buffer: Buffer): Promise<ParsedLog> {
  const state = newParseState()
  const policy = newYieldPolicy()
  const scan = scanZstdFrames(buffer)
  for (const frame of scan.frames) {
    if (Date.now() - policy.lastYield >= YIELD_INTERVAL_MS) await maybeYield(policy)
    let text: string
    try {
      ...
      state.decodeFail = true
      if (state.decodeFailReason === undefined) state.decodeFailReason = `frame-decompress@${frame.start}`
      continue
    }
  ...
  if (scan.tornStart !== undefined) {
    state.decodeFail = true
    if (state.decodeFailReason === undefined) state.decodeFailReason = 'torn-tail'
  }
  if (scan.error !== undefined) {
    state.decodeFail = true
    if (state.decodeFailReason === undefined) state.decodeFailReason = scan.error
  }
...
async function decodeJsonlLog(buffer: Buffer): Promise<ParsedLog> {
  const state = newParseState()
  if (buffer.length === 0) {
    state.decodeFail = true
...
export async function decodeSessionLog(
  path: string,
  compression: Compression,
 ...
 */
export function discoverSessionLogs(root: string): SessionLogFile[] {
  const out: SessionLogFile[] = []
  let projects: Array<{ name: string; isDirectory: () => boolean }>
  try {
    projects = readdirSync(root, { withFileTypes: true }) as unknown as typeof projects
  } catch {
    return out
  }
  for (const project of projects) {
    ...
    let sessionDirs: Array<{ name: string; isDirectory: () => boolean }>
    try {
      sessionDirs = readdirSync(projectPath, { withFileTypes: true }) as unknown as typeof sessionDirs
    } catch {
      continue
    }
 ...
 */
export function decodeSegmentName(name: string): string {
  if (name === '~002E') return '.'
  if (name === '~002E~002E') return '..'
  let out = ''
  let i = 0
  while (i < name.length) {
...
export interface ScanReport {
  ...
  decodes: number
  cacheHits: number
  classes: { A: number; B: number; C: number; D: number }
...
export class ZombieGuardEngine {
  ...
  private readonly warnedKeys = new Map<string, string>()
   ...
   */
  async scanOnce(services: ScanServices, options: ScanOptions): Promise<ScanReport> {
    ...
    const archiveEnabled =
      options.autoArchiveBlank && !options.overrideMode && services.archivedIds !== undefined && options.zstdCapable
    ...
    for (const file of files) {
      ...
      if (cacheHit) report.cacheHits += 1
      else report.decodes += 1

      if (classification.decodeFail) {
        report.decodeFailCount += 1
        const headerOutsideRoster =
          classification.headerPreset === undefined || !roster.has(classification.headerPreset)
        if (headerOutsideRoster) {
          // C 类：解码失败且 headerPreset ∉ roster（header 预设未知也计入——无法给安全结论）。
          report.classes.C += 1
          this.emitWarning(report, options, classification, 'C', `C:${classification.headerPreset ?? '?'}:${classification.decodeFailReason ?? 'decode-fail'}`,
            classification.headerPreset === undefined ? 'header 预设未知（header 不可读或无 agentPreset 字段），依赖不可判' : undefined)
        }
        continue
      }
      ...
      if (live === undefined) {
        ...
        this.emitWarning(report, options, classification, 'A', `A:${classification.effective}:live-unknown`,
          'live? unknown（agents 与 sessions 服务均缺失，不归档）')
        continue
      }
      if (!archiveEnabled) {
        this.emitWarning(report, options, classification, 'A', `A:${classification.effective}:report-only`,
          'report-only（autoArchiveBlank=false / override 模式 / workspaceRegistry 或 zstd 能力缺失），按 B 类告警处理')
        continue
      }
      try {
   ...
   */
  async checkRemove(presetId: string, options: ScanOptions): Promise<CheckRemoveReport> {
    const report: CheckRemoveReport = {
      time: new Date().toISOString(),
      presetId,
      root: options.root,
      scanned: 0,
    ...
    for (const file of files) {
      ...
      if (classification.decodeFail) report.decodeFailCount += 1
      const dependsOn: CheckRemoveMatch['dependsOn'] = []
      if (classification.headerPreset === presetId) dependsOn.push('header(weak)')
      if (!classification.decodeFail && classification.effective === presetId) dependsOn.push('effective(strong)')
  ...
  private async classifyFile(
    file: SessionLogFile,
    options: ScanOptions,
  ): Promise<{ classification: Classification; cacheHit: boolean }> {
    let stamp: Stamp
    try {
   ...
   */
  private pruneStaleEntries(files: SessionLogFile[]): void {
    const livePaths = new Set(files.map((file) => file.path))
    for (const path of this.cache.keys()) {
      if (!livePaths.has(path)) this.cache.delete(path)
    }
    const knownIds = new Set<string>()
    for (const file of files) knownIds.add(decodeSegmentName(file.dirName))
    for (const entry of this.cache.values()) knownIds.add(entry.classification.id)
    for (const [key, id] of this.warnedKeys) {
      if (!knownIds.has(id)) this.warnedKeys.delete(key)
    }
  }
  ...
  private emitWarning(
    ...
    if (firstWarned) this.warnedKeys.set(key, classification.id)
    ...
    if (classification.decodeFail) parts.push(`解码失败(${classification.decodeFailReason ?? '?'})`)
...
function buildClassification(file: SessionLogFile, parsed: ParsedLog): Classification {
  return {
